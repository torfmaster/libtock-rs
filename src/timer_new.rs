    ...
    Self: SubscribableCallback,
{
    pub fn init(&mut self) -> TockResult<Timer> {
        ...
        if num_notifications == 0 {
            // TODO: Wirklich prüfen?
            return Err(TockError::Other("Invalid number of notifiations"));
        }
        ...
        if clock_frequency == 0 {
            return Err(TockError::Other("Erroneous clock frequency"));
        }
            ...
            None => {
                ...
                if duration_ms > freq {
                    match (duration_ms / 1000).checked_mul(freq) {
                        Some(y) => y,
                        None => return Err(TockError::Other("Duration too long")),
                    }
                } else {
